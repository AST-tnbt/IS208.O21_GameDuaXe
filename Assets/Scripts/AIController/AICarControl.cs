...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class AICarControl : MonoBehaviour
    {
        ...
        private AIController m_AIController;    // Reference to actual car controller we are controlling
        ...
        private void FixedUpdate()
        {
            if (m_Target == null || !m_Driving)
            {
                ...
                m_AIController.Move(0, 0, -1f, 1f);
            }
            else
            {
                Vector3 fwd = transform.forward;
                if (m_Rigidbody.velocity.magnitude > m_AIController.MaxSpeed*0.1f)
                {
                    fwd = m_Rigidbody.velocity;
                }

                float desiredSpeed = m_AIController.MaxSpeed;
                    ...
                    case BrakeCondition.TargetDirectionDifference:
                        {
                            ...
                            desiredSpeed = Mathf.Lerp(m_AIController.MaxSpeed, m_AIController.MaxSpeed*m_CautiousSpeedFactor,
                ...
                float accelBrakeSensitivity = (desiredSpeed < m_AIController.CurrentSpeed)
                                                  ? m_BrakeSensitivity
                                                  : m_AccelSensitivity;

                // decide the actual amount of accel/brake input to achieve desired speed.
                float accel = Mathf.Clamp((desiredSpeed - m_AIController.CurrentSpeed)*accelBrakeSensitivity, -1, 1);
                ...
                float steer = Mathf.Clamp(targetAngle*m_SteerSensitivity, -1, 1)*Mathf.Sign(m_AIController.CurrentSpeed);

                // feed input to the car controller.
                m_AIController.Move(steer, accel, accel, 0f);
